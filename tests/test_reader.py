...
indexing2D_tests_unconstrained = [
    (slice(None, None, None), slice(2, 3)),  # [:, 2:3]
    (slice(None, None, None), slice(2, 4)),
]  # [:, 2:4]
...
def test_phys_signals_test_multi_dimslice():
    ...
    x = 1420
    print(eeg.phys_signals.shape)
    res = eeg.phys_signals[
        1:3, x * 200 : x * 200 + 5
    ]  #     eeg.phys_signals[1:3, x*200:x*200+5]

    target = np.array(
        [
            [-69.53124627, -158.49608524, -152.05077308, -168.16405347, -210.74217618],
            [-99.60936965, -78.02733956, -7.81249958, -10.83984317, -35.83984182],
        ]
    )
...
def test_phys_signals_test_slice_int():
    ...
    target = np.array([[49.99999731], [49.99999731]])
...
def test_phys_signals_test_fancy_channel_index():
    ...
    target = np.array(
        [
            [-139.35546126, -140.23436747, -136.32811768, -131.15233671, -125.29296202],
...
def gen_synthetic_signal(num_chan=5, fs=10, duration_sec=2, dig_offsets=None):
    ...
    s2u[-1] = (
        s2u[-1] / 2.0
    )  # make it so at least one channel has a different conversion factor
...
def create_synthetic_eeghdf_file():
    ...
    record = hf.create_record_block(
        record_duration_seconds=T,
